...
class DemoDataGenerator:
    def __init__(self, start_date, end_date, start_user_id=1,
            num_users=100, seed=42):
        np.random.seed(seed)
        random.seed(seed)
    ...
    def generate_user_profile(self, user_id):
        ...
        signup_date = self.start_date + timedelta(days=np.random.randint(0, max(1, total_days)))
    ...
    def generate_datasets(self):
        ...
        for i in range(self.num_users):
            user_id = f'USR{self.start_user_id + i:03d}'
            ...
            if converted and user_activities:  # Only add conversion date if there are activities
                ...
                # Ensure conversion date doesn't exceed end_date
                conversion_date = min(conversion_date, self.end_date)
...
def main():
    parser = argparse.ArgumentParser(description='Generate demo user and activity data')
    parser.add_argument('--start-date', type=parse_date, default='2024-01-01',
                      help='Start date in YYYY-MM-DD format (default: 2024-01-01)')
    parser.add_argument('--end-date', type=parse_date, default='2024-03-31',
                      help='End date in YYYY-MM-DD format (default: 2024-03-31)')
    parser.add_argument('--start-user-id', type=int, default=1,
                      help='Starting user ID number (default: 1)')
    parser.add_argument('--num-users', type=int, default=2000,
                      help='Number of users to generate (default: 2000)')
    parser.add_argument('--prefix', type=str, default='random',
                      help='Prefix for output files (default: random)')
    ...
    # Save to CSV files
    users_df.to_csv(f'{args.prefix}-users.csv', index=False)
    logs_df.to_csv(f'{args.prefix}-logs.csv', index=False)
    ...
    print(f"\nOutput files:")
    print(f"- {args.prefix}-users.csv")
    print(f"- {args.prefix}-logs.csv")
